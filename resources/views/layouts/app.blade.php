...
<head>
    ...
    <title>@yield('title', 'Kelola Users') - Vasco Laundry</title>
    ...
    <style>
        ...
        .sidebar-item.active {
            @apply bg-white text-sky-700 shadow-md;
        }
        ...
        .btn-primary {
            @apply inline-flex items-center gap-2 px-4 py-2 bg-sky-600 text-white text-sm font-semibold rounded-xl hover:bg-sky-700 active:scale-95 transition-all duration-200;
        }

        .btn-secondary {
            @apply inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-200 active:scale-95 transition-all duration-200;
        }
        ...
        .input-field {
            @apply w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition;
        }
        ...
        <aside class="w-64 bg-gradient-to-b from-sky-700 to-sky-900 flex flex-col flex-shrink-0">
            ...
            <div class="px-4 py-3 border-b border-white/10">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <span
                            class="inline-block px-1.5 py-0.5 bg-white/20 text-white/80 text-xs rounded-md capitalize">{{ auth()->user()->role }}</span>
                    </div>
                </div>
            </div>
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('admin.users.index') }}"
                        class="sidebar-item {{ request()->routeIs('admin.users.*') ? 'active' : 'text-sky-100' }}">
                        👤 Kelola User
                    </a>
                @endif
            ...
            <div class="p-4 border-t border-white/10">
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center gap-3 px-4 py-2.5 text-sky-100 hover:text-white hover:bg-white/10 rounded-lg text-sm font-medium transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    @if(session('error'))
                        <div
                            class="flex items-center gap-2 px-3 py-2 bg-red-50 text-red-700 rounded-lg text-sm border border-red-200">
